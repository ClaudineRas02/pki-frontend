...
import { pickFile, pickUploadFile, saveExportFiles } from '../services/fileService';
...
export function ArtifactsScreen({ reloadKey, onDataChanged }) {
  ...
  return (
    <ScreenContainer>
      <TopHero title="Artefacts PKI" subtitle="CA, CSR et CRT generes ou importes" meta="Gestionnaire de fichiers" />
      ...
      <SectionCard title="Importer un artefact">
        ...
        <FormField label="Nom / Common Name" value={importForm.name} onChangeText={(value) => setImportForm({ ...importForm, name: value })} placeholder="api.mondomaine.com" />
        {importForm.type === 'CA' ? (
          <FormField label="Type CA" value={importForm.ca_type} onChangeText={(value) => setImportForm({ ...importForm, ca_type: value })} placeholder="ROOT ou INTERMEDIATE" />
        ) : null}
        {importForm.type === 'CRT' ? (
          <>
            <FormField label="Type CRT" value={importForm.cert_type} onChangeText={(value) => setImportForm({ ...importForm, cert_type: value })} placeholder="SERVER" />
        ...
        <PrimaryButton compact tone="ghost" label={importForm.selectedFile?.name || (importForm.type === 'CA' ? 'Choisir certificat .crt / .pem' : 'Choisir le fichier')} onPress={async () => {
          const selectedFile = importForm.type === 'CA'
        ...
        {importForm.type !== 'CSR' ? (
          <PrimaryButton compact tone="ghost" label={importForm.selectedKeyFile?.name || 'Choisir la cle privee'} onPress={async () => {
            const selectedKeyFile = importForm.type === 'CA'
              ? await pickUploadFile()
              : await pickFile();
            if (selectedKeyFile) {
        ...
        <PrimaryButton label="Importer et ajouter a la liste" onPress={importArtifact} />
      </SectionCard>
      ...
      <SectionCard title="Liste unifiee">
        {artifacts.length ? artifacts.map((item) => (
          <EntityRow
            ...
            title={item.name}
            subtitle={`${item.type} - fichier ${Object.values(item.file_paths || {}).some(Boolean) ? 'present' : 'absent'}`}
            ...
            badgeTone={item.type === 'CA' ? 'accent' : item.type === 'CSR' ? 'info' : 'success'}
            actions={<PrimaryButton compact tone="ghost" label="Exporter" onPress={() => exportArtifact(item)} />}
          />
        )) : (
          <Text style={styles.helper}>Aucun artefact genere ou importe pour le moment.</Text>
        )}
      </SectionCard>
    </ScreenContainer>
  );
