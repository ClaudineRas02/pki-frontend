...
export function DashboardScreen({ reloadKey }) {
  ...
  const activeCertificates = summary.certificates.filter(
  ...
  const revoked = summary.certificates.filter(
  ...
  const recentItems = [
    ...
    .sort(
      (a, b) =>
        new Date(b.created_at || b.issued_at) -
        new Date(a.created_at || a.issued_at),
    )
    .slice(0, 6);
  ...
  const notifications = summary.certificates
    ...
    .filter(
      (item) => item.daysUntil !== null && [7, 30, 90].includes(item.daysUntil),
    )
  ...
  return (
    <ScreenContainer>
      ...
      <SectionCard
        ...
        subtitle="Seuils 7 jours, 30 jours et 90 jours"
      >
        {notifications.length ? (
          notifications.map((item) => (
            <EntityRow
              ...
              subtitle={`ID: ${item.cert_id} - Expire le ${formatDate(item.expires_at)}`}
          ...
          <Text style={styles.empty}>
            Aucune notification a afficher pour les seuils actifs.
          </Text>
        )}
      </SectionCard>

      <SectionCard
        title="Elements recents"
        subtitle="Certificats et authorities les plus recentes"
      >
        {recentItems.map((item) => {
          const isCertificate = item.entityType === "CERT";
          const daysUntil = getDaysUntil(
            isCertificate ? item.expires_at : item.expires_at,
          );
          ...
          return (
            <EntityRow
              ...
              subtitle={
                isCertificate
                  ? `ID: ${item.cert_id} - ${item.cert_type || "CERT"} - ${item.algorithm || "N/A"}`
                  : `ID: ${item.ca_id} - ${item.ca_type || "CA"}`
              }
              meta={
                isCertificate
